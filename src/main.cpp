...
#if defined(NDEBUG)
# error "mula cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xff011dd6f962d21847931757c5f59b73938a5db52b9456f43fca6faf1660832c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // mula: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "mula Signed Message:\n";
...
{
    // mula: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // mula: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // mula: 10 minutes
static const int64 nTargetSpacing = 1 * 60; // mula: 1 minute to mine block
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // mula: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // mula: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x23de73d29c08436afb199444c42ea6189619bfb9700b3fee52b8a4524240df9b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News 28/Feb/2020 Confirmed first coronavirus case in Nigeria";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b87132d993a058844de35a7e0373a0a23bfc99b84e27e10fbc25d89148e19c3de906d7728ae497da14373a6b85bb18457ee74c8db3c91f314c67a366e111b6f1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1583007210;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3771084;

        if (fTestNet)
        {
            block.nTime    = 1583007195;
            block.nNonce   = 3094645;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5d2e8d637c6cda4ae56bc225049693f5835ddd7d114d86cb1a6908c08c8333fb"));
...
unsigned char pchMessageStart[4] = { 0xff, 0xc3, 0xb8, 0xdf }; // mula: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("mulaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("mulaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static mulaMiner(CWallet *pwallet)
{
    printf("mulaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mula-miner");
    ...
    try { loop {
        ...
        printf("Running mulaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&mulaMiner, pwallet));
}
